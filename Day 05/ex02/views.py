...
def populate(request):
	insert_text = """INSERT INTO ex02_movies (title, episode_nb, opening_crawl, director, producer, release_date) VALUES (%s,%s,%s,%s,%s,%s)"""
	inputs = [
...
def display(request):
	fetch_text = """select * from ex02_movies"""
	conn = ''
	try:
		conn = psycopg2.connect(dbname="djangotraining", host='localhost', user="djangouser", password="secret")
	except Exception as e:
	...
	return render(request, 'ex02/view.html', {
		"data": data
	})