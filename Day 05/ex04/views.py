...
def populate(request):
	insert_text = """INSERT INTO ex04_movies (title, episode_nb, opening_crawl, director, producer, release_date) VALUES (%s,%s,%s,%s,%s,%s)"""
	inputs = [
...
def display(request):
	fetch_text = """select * from ex04_movies"""
	conn = ''
	try:
		conn = psycopg2.connect(dbname="djangotraining", host='localhost', user="djangouser", password="secret")
	except Exception as e:
	...
	return render(request, 'ex04/view.html', {
		"data": data
	})
...
def remove(request):
	...
	if request.method == 'POST':
		try:
			curr.execute("DELETE FROM ex04_movies WHERE title=%s", (request.POST['remove'], ))
			conn.commit()
		except Exception as e:
			return HttpResponse("No data available")
